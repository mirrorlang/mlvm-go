...
func call(cpu *Runner) {
	...
	case *mirror.CallAtom: //函数调用,这里必须要知道函数体的rect
		...
		NewFuncX, NewFuncY := cpu.Runfunc.Funcbody.X+cpu.Runfunc.Funcbody.Size_x, cpu.Runfunc.Funcbody.Y
		...
		if caller.Args != nil {
			...
			for i := 0; i < Args.Size_y && i < funcrect.Args.Size_y; i++ {
				for j := 0; j < Args.Size_x && j < funcrect.Args.Size_x; j++ {
					targetatom := cpu.At(NewFuncX+j, NewFuncY+i)
					sourceatom := cpu.At(Args.X+j, Args.Y+i)
					if targetatom != nil {
						//todo 类型检查
						cpu.Set(NewFuncX+j, NewFuncY+i, sourceatom) //参数区是在函数体的x+1位置
					}
	...
	case *mirror.ReturnAtom:
		...
		caller := cpu.At(cpu.Runfunc.Funcbody.X, cpu.Runfunc.Funcbody.Y+cpu.Runfunc.CallerX).(*mirror.CallAtom)
		//存储函数的计算结果
		...
		for i := 0; i < cpu.Runfunc.Value.Size_y; i++ {
			...
			for j := 0; j < cpu.Runfunc.Value.Size_x; j++ {
				cpu.Set(j, i, cpu.InFunc(cpu.Runfunc.Value.X+j, cpu.Runfunc.Value.Y+i))
			}
		}
		//回收执行现场
